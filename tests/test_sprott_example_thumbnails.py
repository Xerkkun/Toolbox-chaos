...
def test_render_example_thumbnail_to_pytest_tmp(tmp_path):
    example = next(
        item for item in load_synthetic_examples()
        if item['id'] == 'synthetic_e_henon_first_beauty'
    )
    output = render_example_thumbnail(example, tmp_path / 'thumb.png')
